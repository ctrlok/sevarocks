...
Одна з причин чому не люблю усіляку логіку в конфігах балансера це складність тестування змін. Тому пишу крізь сльози щастя: можу створювати роутери справжньою мовою під [cloudflare workers](https://developers.cloudflare.com/workers/).
...
Cloudflare workers це V8 [^2], який запускає джаваскрипт код. Якщо ми використовуємо vitest з лібою cloudflare:test то воно вміє запускати движок воркерів, який потім запускає збілджений Rust код і ми просто смикаємо фетч апі (в качці заєць, а в зайці голка).
...
Для простих сценаріїв такого має бути взагалі достатньо. Але ми ж не шукаємо простих шляхів. Наша доля складна і буремна, нам потрібні виклики. Наприклад, виклики бази даних. 
...
Уявімо що у нас є код який рахує скільки разів користувач смикнув воркер і далі виконує якусь логіку на цьому:
...
Це вже складніше тестувати з fetch(), тому що далі логіка може бути досить складною, а кожна варіація збільшує кількість сценаріїв тестування [^3]
...
Для того щоб передати об’єкт бази даних до функції `number_of_requests()` треба спочатку згенерити біндінги [^4]

[^4]: Уяви, що в тебе є два друга які спілкуються різними мовами. Rust каже "я контролюю пам'ять", а JavaScript каже "я взагалі не знаю що таке пам'ять, але у мене є undefined". І от вони хочуть разом щось зробити. Біндінги — це той перекладач, який сидить між ними й перекладає їхні божевільні розмови. Хоча я б таких друзів в дурку здав. І залишився б без друзів. 

Для цього можна використати чинні макроси з `wasm_bindgen`. Тоді для такої функції: 
...
Але наша функція `async fn number_of_requests(db: D1Database, user_id: i32) -> Result<i32>` приймає базу даних як вхідний параметр. І якщо ми додамо макрос `#[wasm_bindgen] то отримаємо помилку 
...
Тобто ми точно очікуємо лише базу даних, тому на вхід отримуємо якийсь об’єкт, який потім конвертуємо в базу даних [^5].

[^5]: можливо і потрібно створювати біндінги під об’єкти у вашому rust коді. 
	Наприклад
	```rust
	#[cfg(debug_assertions)]
