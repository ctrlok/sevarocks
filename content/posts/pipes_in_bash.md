...
Я знаю, знаю, баш як мова це стрьом і спроектовано дуже давно, людьми, деякі з яких жили ще при фараонах і вже поховані у пірамідах. 

Але, **але!** вислухайте мене. Люди які писали баш були **розумними** людьми. І вони робили **цікаві** і *практичні* речі. Дуже багато цікавих і пракичних речей. 
...
1. Збілдити докер імеджі з переліку
...
Доречі, класно пушити у різні реджестрі можна за допомогою [regctl](https://github.com/regclient/regclient/blob/main/docs/regctl.md) але не в моєму випадку, тому що мені було потрібно: 

1. Збілдити досить великі докер імаджі з нейронками
...
Обмеження в тому що можемо білдити і експортити тільки один імедж одночасно, також віддалені реджестрі мають обмеження на пуш — можно пушити тільки один імедж одночасно.

Але це дуже багато контексту, тому я зникав це під спойлер для допитливих. Треба не забути зробити аналітику по кількості людей які кликнули на спойлер, знаючи вас, має бути 99.9% 

{% end %}

Усе просто і в наївному варіанті це має виглядати якось так: 
...
Якщо мінуси наївного підходу не критичні, то __завжди__ треба використовувати наївний підхід з мінімумом оптимізацій. 
...
export -f webhook
```
У майбутньому ми просто замінимо тіло функції на щось валідне, а так будемо мати змогу додати будь-який лог і навіть протестувати логіку (але це тестування баш скриптів це тема для окремого поста)
{% end %}
...
# Також ми можемо очікувати на завершення задачі знаючи її PID

last_task_pid=$!
wait $last_task_pid

# Чи можемо чекати доки не виконається усе що було у фоні
wait
```
...
Ми знаємо що якщо написати `ls | grep` то усі данні які надрукує `ls` в `stdout` підуть до команди `grep`. 


Це відбувається тому що кожен процес в лінуксі типово має як мінімум три відритих файлових дескриптори (якщо потім самостійно їх не закриє) - stdin (0), stdout (1), stderr (2) і пайпи це просто перенаправлення данних з одного дескриптора в інший. 

Ми могли б замість пайпів використати і файли, але пайпи зручні тим що це ще й існтрумент синхронізації: коли пайп закривається, то процесс який з нього читає розуміє що більше інформації нема і можна спокійно повзти в помиральну яму. 

Щоб краще зрозуміти пайпи, рекомендую почитати [код в ядрі](https://github.com/torvalds/linux/blob/059dd502b263d8a4e2a84809cf1068d6a3905e6f/fs/pipe.c) і подивитись як можна імпліментувати баш пайпи в простій программі на Сі
...
У прикладі далі ви побачите `exec 3>/tmp/reg1.fifo` — це перенаправлення третього (і нового файлового дескриптора) в іменований пайп. Це потрібно для того щоб не перевідкривати пайп, а відкрити його тільки один раз і тримати відкритим доки не закриємо. 
...
Таким чином ми у фоні запустили процесс який незалежно білдить імеджі і крізь пайпи сповіщає інші два процесси які по одному, незалежно один від одного, пушать імеджі до реджестрі. 

Усе що залишилось це сповістити про успіх і зробити вебхук

# Синхронізація незалежних процессів

Якщо те що було вище здалося вам оверінженерінгом, то заплющуйте очі. 

{% foldable(title="Адекватна реалізація") %}

Синхронізація не те щоб потрібна в цьому прикладі, тому ми могли б просто смикнути усі вебхуки після команди `wait`, тобто коли ми вже точно впечнені що усе запушено. Але можуть бути інші задачі, коли треба щось запустити після кожної ітерації, а не після усіх. 

{% end %}

Як є багато способів освіжувати кота, так і синхронізувати потоки можна декількома способами: за допомогою сигналів і команди `trap`, за допомогою лок файлів, за допомогою команди `wait` і за допомогою пайпів. 

Як на мене, то пайпи це найпростіша і найшвидша реалізація, але якщо вам цікаві інші способи (чи ви знаєте інші способи) то лишайте коментарі. 

## Синхронізація пайпами

Що ми знаємо про пайпи? Ну, як мінімум, що якщо ми намагаємось прочитати з пайпа в який ніхто не пише, то будемо чекати вічність. 
